...
- (void)clearCachedStatements {
    ...
    for (YYFMStatement *cachedStmt in [_cachedStatements objectEnumerator]) {
        [cachedStmt close];
    }
...
- (void)closeOpenResultSets {
    ...
    NSMutableSet *openSetCopy = YYFMDBReturnAutoreleased([_openResultSets copy]);
...
- (void)warnInUse {
    ...
    if (_crashOnErrors) {
        abort();
        NSAssert1(false, @"The YYFMDatabase %@ is currently in use.", self);
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        if (_crashOnErrors) {
            abort();
            NSAssert1(false, @"The YYFMDatabase %@ is not open.", self);
        }
    #endif
...
- (NSError*)errorWithMessage:(NSString*)message {
    ...
    return [NSError errorWithDomain:@"YYFMDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];    
}
